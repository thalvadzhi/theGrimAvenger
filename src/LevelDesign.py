...
camera = [Camera(GAME_MEASURES[0], GAME_MEASURES[1], WIDTH - WIDTH // 4, HEIGHT)]
...
expand_up = Button((WIDTH - WIDTH // 4 + 10, HEIGHT - 90), (100, 30), "Expand^", (0, 0, 0), (255, 255, 255))
expand_right = Button((WIDTH - WIDTH // 4 + 10, HEIGHT - 30), (100, 30), "Expand>", (0, 0, 0), (255, 255, 255))

retract_left = Button((WIDTH - WIDTH // 4 + 110, HEIGHT - 30), (100, 30), "Retract<", (0, 0, 0), (255, 255, 255))
retract_down = Button((WIDTH - WIDTH // 4 + 110, HEIGHT - 90), (100, 30), "Retract\/", (0, 0, 0), (255, 255, 255))

delete_button = Button((WIDTH - WIDTH // 4 + 65, HEIGHT - 140), (100, 30), "Despawn", (0, 0, 0), (255, 255, 255))
save_button = Button((WIDTH - WIDTH // 4 + 65, HEIGHT - 200), (100, 30), "Save", (0, 0, 0), (255, 255, 255))
load_button = Button((WIDTH - WIDTH // 4 + 65, HEIGHT - 280), (100, 30), "Load", (0, 0, 0), (255, 255, 255))

quit = Button((WIDTH - WIDTH // 4 + 65, HEIGHT - 340), (100, 30), "Quit", (0, 0, 0), (255, 255, 255))

buttons = [rectangle_button, sawblock_button, shadow_button, expand_up, expand_right, retract_down, retract_left,
           delete_button, save_button, load_button, quit]
##############################
...
def construct():
    ...
    camera.clear()
    camera.append(Camera(GAME_MEASURES[0], GAME_MEASURES[1], WIDTH - WIDTH // 4, HEIGHT))
    Shadow.set_up(GAME_MEASURES[0], GAME_MEASURES[1])


def set_up():
    Shadow.set_up(GAME_MEASURES[0], GAME_MEASURES[1])
    world.append(Block((0, 0, 0), GAME_MEASURES[0], 32, 0, 0))
    world.append(Block((0, 0, 0), GAME_MEASURES[0], 32, 0, GAME_MEASURES[1] - 32))
    world.append(Block((0, 0, 0), 32, GAME_MEASURES[1], 0, 0))
    world.append(Block((0, 0, 0), 32, GAME_MEASURES[1], GAME_MEASURES[0] - 32, 0))

def move_down():
    for index, item in enumerate(world):
        if index >= 4:
            if isinstance(item, Block):
                item.rect.center = Vector(item.rect.center[0], item.rect.center[1] + 32)
                ...
                item.bottomleft = (item.bottomleft[0], item.bottomleft[1] + 32)
                item.bottomright = (item.bottomright[0], item.bottomright[1] + 32)
...
def resize_game_field(action):
    if action == "expand right":
        ...
        return
    #game_height += 0
    camera[0] = Camera(GAME_MEASURES[0], GAME_MEASURES[1], GAME_MEASURES[2], GAME_MEASURES[3])
    set_up()
    for i in range(4):
...
def save():
    level = {"constants": CONSTANTS, "game measures": GAME_MEASURES, "world": world}
...
def spawn(figure="none", size=(0, 0, (0, 0, 0))):
    if size == -1:
        ...
        world.append(Block((0, 0, 0), size[0], size[1], camera[0].reverse_apply((32, 32))[0],
                           camera[0].reverse_apply((32, 32))[1]))
    elif figure == "sawblock":
        world.append(SawBlock(camera[0].reverse_apply((50, 0))[0], camera[0].reverse_apply((50, 0))[1], size))
    elif figure == "shadow":
        world.append(Shadow(camera[0].reverse_apply(size[0]), camera[0].reverse_apply(size[1]), camera[0].reverse_apply(size[2])
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            for index, piece in enumerate(world):
                if isinstance(piece, Shadow):
                    if piece.collide([camera[0].reverse_apply(mouse_position)]):
                        return index
...
def resize_shadow(shadow, mouse_position):
    ...
    for rect in rects:
        if rect.is_point_in_body(mouse_position, camera[0]):
            ...
            shadow.topright = top_right_rect.center[0], top_right_rect.center[1]
            shadow.bottomleft = bottom_left_rect.center[0], bottom_left_rect.center[1]
            shadow.bottomright = bottom_right_rect.center[0], bottom_right_rect.center[1]
...
def move(index):
    ...
    while True:
        ...
        if isinstance(world[index], Block):
            world[index].rect.position_m = Vector(camera[0].reverse_apply(mouse_position))
        elif isinstance(world[index], SawBlock):
            world[index].rect.center = Vector(camera[0].reverse_apply(mouse_position))
